...
double PlainVanillaOption::bsprice(double s, double k, double r,
...
double PlainVanillaOption::bsvega(double s, double k, double r,
                                  double q, double t, double sigma, OptionType type) {
    double d1 = (log(s/k) + (r - q + 0.5 * sigma*sigma) * t) / (sigma * sqrt(t));
...
double PlainVanillaOption::price(Date evalDate) {
    ...
    double s = p_.getSpot();
    double r = p_.getRf().rate(expiration_);
    double q = p_.getDiv().rate(expiration_);
...
double PlainVanillaOption::impliedVol(Date evalDate, double mktPrice,
